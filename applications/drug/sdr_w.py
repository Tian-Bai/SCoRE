...
# X_train = np.column_stack([X_train, df_train['mw']])
...
# X_calibtest = np.column_stack([X_calibtest, df_calibtest['mw']])
...
if not os.path.exists('sdr_results_w'):
    os.makedirs('sdr_results_w')

all_res.to_csv(os.path.join('sdr_results_w', f"SCoRE_SDR_w, data={data_name}, model={model}, Nrep={Nrep}, seedgroup={seedgroup}, reward={reward_type}, oracle={oracle}.csv"))
